...
export const ParentCategoryForm = ({
  ...
  setParentCategories: React.Dispatch<React.SetStateAction<any>>;
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data?.data?.id) {
        ...
        setParentCategories((prev: any) => [data.data, ...prev]);
        setIsAdding(false);
      } else {
    ...
    <>
      <div className='max-w-2xl mx-auto p-6 md:p-8'>
        <div className='space-y-4'>
          <div>
            <h1 className='text-2xl font-bold'>Create Parent Category</h1>
